...
\begin{itemize}
    \item \textbf{Address Bar Features}
    \item \textbf{Abnormal-based Features}
    \item \textbf{HTML and JavaScript-based Features}
    \item \textbf{Domain-based Features}
\end{itemize}

Each feature is automatically assigned a discrete value, such as $-1$ (\textit{legitimate}), $0$ (\textit{suspicious}), or $1$ (\textit{phishing}), based on heuristic rules derived from observed URL statistics. Examples include:

\begin{itemize}
    \item \texttt{having\_ip\_address}: whether the URL contains an IP address. $\{-1, 1\}$
    \item \texttt{url\_length}: the length of the URL string. $\{-1, 0, 1\}$
    \item \texttt{shortening\_service}: whether a URL shortening service was used. $\{-1, 1\}$
\end{itemize}
...
The 30 extracted features are stored in a 30-dimensional vector, represented as a \texttt{torch.Tensor}, which serves as the sole input to the neural network model.
